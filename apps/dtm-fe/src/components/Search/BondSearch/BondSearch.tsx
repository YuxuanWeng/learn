...
import { BondOptionRender } from '@fepkg/business/components/Search/BondSearch';
...
import { BaseDataBondSearch } from '@fepkg/services/types/base-data/bond-search';
import { FiccBondBasic, QuoteParsing, Trader } from '@fepkg/services/types/common';
import { BondSearchType, FiccBondInfoLevel } from '@fepkg/services/types/enum';
...
export const BondSearch = forwardRef<HTMLInputElement, BondSearchProps>(
  (
    ...
    // 信用债需要展示评级-避免传入的optionRender不生效
    optionRender = useMemoizedFn(optionRender ?? BondOptionRender(productType, true));
    ...
    const query = useFuzzySearchQuery<BaseDataBondSearch.Response, BaseDataBondSearch.Request>({
      api: APIs.baseData.bondSearch,
      keyword: bondSearchState.keyword,
      searchParams: {
        product_type: !productTypeList ? productType : void 0,
